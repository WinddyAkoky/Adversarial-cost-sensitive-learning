...
# 参数
parser = argparse.ArgumentParser(description="MNIST CSA Training")
...
# 记录日志
path_log = '../log/Lenet_MNIST_CSA_L2.log'
...
def run_training():
    ...
    # criterion = Loss_cost_sensitive_L2()

    # 循环 对每一个类分别进行保护
    for i_label in range(1):
        # 读取预训练模型
        ...
        criterion = Loss_cost_sensitive_L2(model=model_cost_sensitive, weight_decay=1e-2)
    ...
    #             
            ...
            if correct/len(test_loader.dataset) > correst_adv_best:
                correst_adv_best = correct/len(test_loader.dataset)
                 # 保存模型
                if NORMALIZE:
                    path_model_save = '../model_final/LeNet_MNIST_adv_cost_sensitive_L2_' + str(i_label) +'.pt'
                else:
                    pass
                    # path_model_save = '../model_final/LeNet_FASHION_MNIST_unnormalized_adv_cost_sensitive_L2_' + str(i_label) + '_e' + str(epsilon) +'.pt'
