...
# 记录日志
path_log = '../log/Lenet_MNIST_CSA.log'
    ...
    #             
            ...
            if correct/len(test_loader.dataset) > correst_adv_best:
                correst_adv_best = correct/len(test_loader.dataset)
                 # 保存模型
                if NORMALIZE:
                    path_model_save = '../model_final/LeNet_MNIST_adv_cost_sensitive_' + str(i_label) +'.pt'
                else:
                    pass
                    # path_model_save = '../model_final/LeNet_FASHION_MNIST_unnormalized_adv_cost_sensitive_' + str(i_label) + '_e' + str(epsilon) +'.pt'
