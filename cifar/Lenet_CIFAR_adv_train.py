...
# 记录日志
path_log = '../log/Lenet_CIFAR_adv_train.log'
...
def save_model():
    if NORMALIZE:
        path_model = os.path.join(model_path_home, 'LeNet_CIFAR_adv_e'+str(epsilon)+'.pt')
    else:
        path_model = os.path.join(model_path_home, 'LeNet_CIFAR_unnormalized_adv_e'+str(epsilon)+'.pt')
