...
# 参数
parser = argparse.ArgumentParser(description="FASHION_MNIST Training")
...
# 记录日志
path_log = '../log/Lenet_FASHION_MNIST_adv_train.log'
...
def save_model():
    if NORMALIZE:
        path_model = os.path.join(model_path_home, 'LeNet_FASHION_MNIST_adv_e'+str(epsilon)+'.pt')
    else:
        path_model = os.path.join(model_path_home, 'LeNet_FASHION_MNIST_unnormalized_adv_e'+str(epsilon)+'.pt')
